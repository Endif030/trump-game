...
export const useGameStore = create<GameStore>()(persist((set, get) => ({
  ...
  makeChoice: (levelId, optionId, result) => set((state) => {
    ...
    // Calculate discovery chance based on secrecy attribute
    const secrecyBonus = state.attributes.secrecy * 10; // -10% per secrecy point
    const baseDiscoveryChance = result.investigationChange + state.investigation * 0.5;
    const finalDiscoveryChance = Math.max(0, baseDiscoveryChance - secrecyBonus);
    
    let newDiscoveredCount = state.discoveredCount;
    if (result.insiderTrading && Math.random() * 100 < finalDiscoveryChance) {
      newDiscoveredCount++;
    }
    ...
    return {
      ...
      history: [...state.history, { 
        ...
        levelName: levelId, 
        choice: optionId, 
        assetsChange: result.assetsChange, 
        prestigeChange: result.prestigeChange, 
        investigationChange: result.investigationChange 
      }],

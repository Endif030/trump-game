  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-5xl mx-auto p-4">
        ...
        {/* Starting Funds - Split display */}
        <motion.div className="bg-white/10 backdrop-blur-md rounded-lg p-4 mb-4 text-center border border-white/20"
          initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        >
          <div className="grid grid-cols-3 gap-4">
            <div>
              <p className="text-white/60 text-xs mb-1">{language === 'zh' ? '初始资金' : 'Base Funds'}</p>
              <p className="text-xl font-bold text-white">$5,000,000</p>
            </div>
            <div>
              <p className="text-white/60 text-xs mb-1">{language === 'zh' ? `点数资金 (${remainingPoints}×$20万)` : `Points Funds (${remainingPoints}×$200K)`}</p>
              <p className="text-xl font-bold text-yellow-400">+${(remainingPoints * 200000).toLocaleString()}</p>
            </div>
            <div className="border-l border-white/20 pl-4">
              <p className="text-white/60 text-xs mb-1">{language === 'zh' ? '总计' : 'Total'}</p>
              <p className="text-2xl font-bold text-green-400">${(5000000 + remainingPoints * 200000).toLocaleString()}</p>
            </div>
          </div>
        </motion.div>
